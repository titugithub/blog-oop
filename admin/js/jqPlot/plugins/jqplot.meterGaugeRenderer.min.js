...
(function (c) {
  ...
  c.jqplot.MeterGaugeRenderer.prototype.constructor =
  ...
  c.jqplot.MeterGaugeRenderer.prototype.init = function (e) {
    ...
    if (this.ticks.length) {
      ...
      this.max =
        this.max == null ? this.ticks[this.ticks.length - 1][0] : this.max;
      this.setmin = false;
      this.setmax = false;
      this.numberTicks = this.ticks.length;
      this.tickInterval = this.ticks[1][0] - this.ticks[0][0];
      this.tickFactor = Math.floor(
        parseFloat((Math.log(this.tickInterval) / Math.log(10)).toFixed(11))
      );
      this.numberMinorTicks = b(
        this.tickPositions,
        this.tickInterval,
        this.tickFactor
      );
  ...
  c.jqplot.MeterGaugeRenderer.prototype.draw = function (X, aC, ap) {
    ...
    if (this.label) {
      this._labelElem = c(
        '<div class="jqplot-meterGauge-label" style="position:absolute;">' +
          this.label +
          "</div>"
      );
      ...
      if (this.semiCircular) {
        if (Q >= 2 * ab) {
          ...
          this.needleThickness =
            this.needleThickness || 2 + Math.pow(this.ringWidth, 0.8);
          this.innerPad =
            this.ringWidth / 2 + this.needleThickness / 2 + this.needlePad;
    ...
    if (this.ticks.length == 0) {
      ...
      var aw = Math.floor(
        parseFloat((Math.log(au) / Math.log(10)).toFixed(11))
      );
      ...
      if (!q && !aG) {
        ...
        aw =
          Math.floor(parseFloat((Math.log(P) / Math.log(10)).toFixed(11))) - 1;
    ...
    if (this.semiCircular) {
      var z = Math.atan(this.innerPad / U),
        ...
        B = (this.hubStartAngle =
          aE - Math.atan((this.innerPad / this.hubRadius) * 2)),
        af = (this.hubEndAngle =
          k + Math.atan((this.innerPad / this.hubRadius) * 2));
      X.save();
      ...
      for (var aa = 0; aa < this.shadowDepth; aa++) {
        X.translate(
          this.shadowOffset * Math.cos((this.shadowAngle / 180) * Math.PI),
          this.shadowOffset * Math.sin((this.shadowAngle / 180) * Math.PI)
        );
        X.beginPath();
        X.strokeStyle = aj;
      ...
      if (this.showTicks) {
        ...
        for (aa = 0; aa < this.ticks.length; aa++) {
          ...
          X.lineTo(
            -(f - aq) * Math.cos(am * aa + aE),
            (f - aq) * Math.sin(am * aa + aE)
          );
          ...
          if (aa < this.ticks.length - 1) {
            for (var Y = 1; Y <= F; Y++) {
              X.beginPath();
              X.moveTo(
                -f * Math.cos(am * aa + p * Y + aE),
                f * Math.sin(am * aa + p * Y + aE)
              );
              X.lineTo(
                -(f - v) * Math.cos(am * aa + p * Y + aE),
      ...
      if (this.showTickLabels) {
        ...
        for (aa = 0; aa < this.ticks.length; aa++) {
          J = c(
            '<div class="jqplot-meterGauge-tick" style="position:absolute;">' +
              this.ticks[aa][1] +
              "</div>"
          );
          ...
          W =
            this._tickPoints[aa][0] -
            (aO * (this._tickPoints[aa][2] - Math.PI)) / Math.PI -
            an * Math.cos(this._tickPoints[aa][2]);
          T =
            this._tickPoints[aa][1] -
            g / 2 +
            (g / 2) *
              Math.pow(Math.abs(Math.sin(this._tickPoints[aa][2])), 0.5) +
            (an / 3) *
              Math.pow(Math.abs(Math.sin(this._tickPoints[aa][2])), 0.5);
          J.css({ left: W, top: T });
          G =
            aO * Math.cos(this._tickPoints[aa][2]) +
            g * Math.sin(Math.PI / 2 + this._tickPoints[aa][2] / 2);
          n = G > n ? G : n;
        }
      }
      if (this.label && this.labelPosition == "inside") {
        ...
        var an = this.tickPadding * (1 - 1 / (this.diameter / 80 + 1));
        var T =
          0.5 * (this._center[1] + this.canvas._offsets.top - this.hubRadius) +
          0.5 *
            (this._center[1] +
              this.canvas._offsets.top -
              this.tickOuterRadius +
              this.tickLength +
              an) +
      ...
      for (aa = 0; aa < this.intervals.length; aa++) {
        y =
          aa == 0 ? aE : aE + ((this.intervals[aa - 1][0] - this.min) * u) / P;
        if (y < 0) {
          y = 0;
        }
      ...
      var al =
        (((ay - this.min) / R) * this.span * Math.PI) / 180 + this.startAngle;
      X.save();
      X.beginPath();
      X.fillStyle = this.ringColor;
      X.strokeStyle = this.ringColor;
      ...
      this.needleThickness =
        this.needleThickness < 2 ? 2 : this.needleThickness;
  ...
  c.jqplot.MeterGaugeAxisRenderer.prototype.constructor =
  ...
  c.jqplot.MeterGaugeLegendRenderer.prototype =
    new c.jqplot.TableLegendRenderer();
  c.jqplot.MeterGaugeLegendRenderer.prototype.constructor =
    c.jqplot.MeterGaugeLegendRenderer;
    ...
    if (this.show) {
      ...
      x +=
        this.marginBottom != null
          ...
          : "";
      x +=
        this.marginLeft != null ? "margin-left:" + this.marginLeft + ";" : "";
      x +=
        this.marginRight != null
          ? "margin-right:" + this.marginRight + ";"
          : "";
      this._elem = c(
        '<table class="jqplot-table-legend" style="' + x + '"></table>'
      );
      var f = false,
        q = false,
        u,
      ...
      if (w.show) {
        ...
        for (n = 0; n < u; n++) {
          if (q) {
            r = c('<tr class="jqplot-table-legend"></tr>').prependTo(
              this._elem
            );
          } else {
          ...
          for (m = 0; m < o; m++) {
            if (v < t.length) {
              ...
              g = c(
                '<td class="jqplot-table-legend" style="text-align:center;padding-top:' +
                  k +
              ...
              e = c(
                '<td class="jqplot-table-legend" style="padding-top:' +
                  k +
                  ';"></td>'
              );
              if (this.escapeHtml) {
  ...
  function a(j, h, f) {
    ...
    if (e) {
      ...
      f.gridPadding.bottom =
        f.gridPadding.bottom != null ? f.gridPadding.bottom : 0;
      f.gridPadding.left = f.gridPadding.left != null ? f.gridPadding.left : 0;
      f.gridPadding.right =
        f.gridPadding.right != null ? f.gridPadding.right : 0;
    }
  }
  function d(e) {}
  ...
  c.jqplot.MeterGaugeTickRenderer.prototype.constructor =
    c.jqplot.MeterGaugeTickRenderer;
})(jQuery);
